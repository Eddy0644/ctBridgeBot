...
async function onTGMsg(tgMsg) {
    ...
    try {
        ...
        if (tgMsg.voice) {
            ...
            tgBotDo.SendChatAction("record_voice", tgMsg.matched).then(() => {});
                ...
                if (mapPair[0] === tgMsg.reply_to_message.message_id/*fixme also verify chat_id here*/ && mod.tgProcessor.isSameTGTarget(mapPair[4], tgMsg.matched)) {
                    ...
                    if (tgMsg.text === "@") {
                        ...
                        if (tgMsg.matched.s === 0) {
                            ...
                            tgBotDo.SendChatAction("choose_sticker", tgMsg.matched).then(() => {});
        ...
        switch (tgMsg.text) {
            ...
            case "/info" + botName: {
                ...
                tgBotDo.SendChatAction("typing", tgMsg.matched).then(() => {});
...
async function deliverTGToWx(tgMsg, tg_media, media_type) {
    ...
    tgBotDo.SendChatAction(action, receiver).then(() => {});
    ...
    tgBotDo.SendChatAction("record_video", receiver).then(() => {});
    if (s === 0) {
    ...
    tgBotDo.SendChatAction("choose_sticker", receiver).then(() => {});
    return true;
}

async function findSbInWechat(token, alterMsgId = 0, receiver) {
    const s = alterMsgId === 0;
    tgBotDo.SendChatAction("typing", receiver).then(() => {});
...
async function getFileFromWx(msg) {
    try {
        ...
        if (fs.existsSync(filePath)) {
            ...
            tgBotDo.SendChatAction("upload_document").then(() => {});
