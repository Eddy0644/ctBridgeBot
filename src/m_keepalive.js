...
async function triggerCheck() {
    ...
    if (isNowBetween) {
        if (secret.misc.debug_show_additional_log) console.log("Report on keepalive.triggerCheck #1: ", t_stv.msgCounter_prev, " ", state.v.wxStat.MsgTotal);
        ...
        if (t_stv.msgCounter_prev < state.v.wxStat.MsgTotal) {
            ...
            const idle_length = dayjs().unix() - t_stv.idle_start_ts;
            const idle_max = t_conf.trigger_v1[0].max_idle_minutes * 60;
            if (t_stv.idle_start_ts !== 0 && idle_length > idle_max) {
                // Idle timer exceeds, so trigger keepalive
                ctLogger.info(`Keepalive triggered: last update of idle timer is ${dayjs(t_stv.idle_start_ts).format("HH:mm")}, which exceeds ${t_conf.trigger_v1[0].max_idle_minutes} minutes from now.`);
...
async function check_byAvatarUrl() {
    ...
    // TODO compare with default avatar, after collecting data
}
